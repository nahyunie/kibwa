...
var promise = mongoose.connect('mongodb+srv://testdb:testdbpassword@cluster0.y1ucp.mongodb.net/HighScoreDB?retryWrites=true&w=majority',{
    useMongoClient:true
});
  ...
  app.get('/page-home', function(request, response) {
    ...
    app.get('/page-quiz5', function(request, response) {
      ...
      app.get('/page-mypage', function(request, response) {
        ...
        var context = {name: myname, email: nhemail, telephone: nhtelephone, school: nhschool, quiz : quizCorrect,sign: nhquizsign, check : myCheck};
        back='page-mypage';
      ...
      app.get('/login', function(request, response) {
        ...
        var context = {myid: nhname,name: myname, email: nhemail, telephone: nhtelephone, school: nhschool, quiz : quizCorrect,sign:nhquizsign, check : myCheck};
            request.app.render('page-login',context, function(err,html){
                if(err){
                    console.error('뷰 렌더링 중 오류 발생:' + err.stack);
      ...
      app.get('/page-logout', function(request, response) {
        ...
        || element=='page-quiz' || element=='page-modification'||element=='page-chatting'
        ||element=='page-board-view'||element=='page-board-view/:id'||element=='page-board-view/delete'
        ||element=='edit/:id'||element=='delete/:id'||element=='exam'||element=='comment/write'||element=='edit-success/:id'){
            response.writeHead('200', {'Content-Type': 'text/html;charset=utf8'});
            response.write("<script>alert('로그인 후 사용 가능합니다'); </script>");
            request.app.render('page-login',context, function(err,html){
                if(err){
      ...
      app.get('/page-modification', function(request, response) {
        ...
        var context = {name: myname, email: nhemail, telephone: nhtelephone, school: nhschool, sign: nhquizsign,quiz : quizCorrect};
        ...
        if(database){
            modification(database, paramName, paramEmail, paramTelephone, paramSchool, function(err, result){
                ...
                if(result){
                    ...
                    if(quizCorrect>=70){
                        myCheck=1;}
                     var context = {name: myname, email: nhemail, telephone: nhtelephone, school: nhschool,sign: nhquizsign, quiz : quizCorrect, check : myCheck};
...
app.post('/login',function(request, response) {
    ...
    paramId = request.body.id || request.query.id;
    paramPassword = request.body.password || request.query.password;
        ...
        authUser(database, paramId, paramPassword, function(err, docs){
            ...
            if(docs){
               ...
               if(quiz3==0){
                ...
                var context = {myid: nhname,name: myname, email: nhemail, telephone: nhtelephone, school: nhschool ,sign: nhquizsign, quiz : quizCorrect, check : myCheck, chart1bar : quiz1code, chart2bar:quiz2code, chart3bar:quiz3code, chart4bar:quiz4code, chart5bar:quiz5code,
                    chart1prog: quiz1codeScore, chart2prog: quiz2codeScore, chart3prog: quiz3codeScore, chart4prog: quiz4codeScore, chart5prog: quiz5codeScore,
                    chart1in: quiz1codeInco, chart2in: quiz2codeInco, chart3in: quiz3codeInco, chart4in: quiz4codeInco, chart5in: quiz5codeInco,
                    coNum1 : quiz1per, coNum2 : quiz2per, coNum3 : quiz3per, coNum4 : quiz4per, coNum5 : quiz5per, 
                    incoNum1 : inco1, incoNum2 : inco2, incoNum3 : inco3, incoNum4 : inco4, incoNum5 : inco5,
 ...
 var addUser = function(database, name, id,  password, email, telephone, school, callback){
     ...
     users.insertMany([{"name" : name, "_id": id, "password": password, "email": email, "telephone": telephone, "school": school, "quizsign":'null'}], function(err, result){
         if(err){
             console.log('1763');
             callback(err, null);
             return;
     ...
     webQuizScore.insertMany([{"_id" : id, "quizScore1" : 0, "quizScore2" : 0, "quizScore3" : 0, "quizScore4" : 0, "quizScore5" : 0}], function(err,result){
         if(err){
             callback(err,null);
             return;
         }
 ...
 function connectDB(){
     var databaseUrl ='mongodb+srv://testdb:testdbpassword@cluster0.y1ucp.mongodb.net/HighScoreDB?retryWrites=true&w=majority';
    ...
    console.log('connection info :', socket.request.connection._peername);
     ...
     socket.on('login', function(login) {
        ...
        console.dir(login);
         ...
         myarr[myarri]=socket.id;
         login_ids[login.id] = socket.id;
         socket.login_id = login.name;
         namearr[myarri]=login.name;
         myarri++;
   
         console.log('2140 사용자 이름' +login.name);
         console.log('2131 : %d',Object.keys(myarr).length);
         console.log('접속한 클라이언트 ID 갯수 : %d', Object.keys(login_ids).length);
 
         //io.sockets.emit('update', {type: 'connect', name: 'SERVER', message: login + '님이 접속하였습니다.'})
         // 응답 메시지 전송
         sendResponse(socket, 'login', '200', login.name);
         
     });
 
     socket.on('disconnect', function() {
        console.log('2141 이벤트를 받았습니다.');
         //console.dir(data+'2142');
         
         sendResponse(socket, 'disconnect', '200', socket.login_id);
         //socket.broadcast.emit(socket, 'login', '200', '로그인되었습니다.');
     });
     ...
     socket.on('message', function(message) {
        ...
        console.dir(message);
         ...
         if (message.recepient =='ALL') {
            ...
            var myhour=moment().format('HH');
             var mymin=moment().format('mm');
           
             var mymsg = {command: message.command, recepient:message.recepient, data: message.data , arr: myarr, hh:myhour,mm:mymin,
                  myarr:myarr, namearr: namearr, mysocket:socket.id};
             socket.broadcast.emit('message',mymsg);
         }
 ...
 function sendResponse(socket, command, code, message) {
     var statusObj = {command: command, code: code, message: message , arr: myarr};
     
     if(command=='login'){
         socket.broadcast.emit('response', statusObj);
     }
